...
| DAC | PCM5102A 立体声 I2S DAC 模块 |
| 功放 | ZK-502C（TPA3116D2，双声道 D 类，板载 BT 5.0） |
...
| 电压分压 | R1=220kΩ + R2=47kΩ（电池电量 ADC 检测） |
...
12V 电池 ──┬──→ ZK-502C VCC（直接 12V 供电）
           ...
           │
           └──→ 电池电量分压器：
                  12V+ ──[R1 220kΩ]──┬──[R2 47kΩ]── GND
                                      │
                                      └──→ GPIO7 (PIN_BATT_ADC)
```

> 分压后最高 ≈ 2.22V（12.6V 满电），在 ESP32-S3 ADC 3.3V 量程内安全。
> R1/R2 使用 1% 精度电阻可提高测量准确度。
