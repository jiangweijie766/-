...
| ⚡ **电量检测 R1** | **220kΩ 电阻，1/4W**（1% 或 5% 精度均可）| **⚠️ 需新购** |
| ⚡ **电量检测 R2** | **47kΩ 电阻，1/4W**（1% 或 5% 精度均可）| **⚠️ 需新购** |

> **关于 R1/R2**：这两只电阻组成分压器，把 12V 电池电压降到 2.22V 以内，让 ESP32-S3 的 ADC 可以安全测量电量。成本极低（合计 < ¥1），但缺少它们屏幕上的电量图标将无法正常工作。
...
> ⚠️ **需要新增硬件：电池电量检测分压器**
>
> 原来的硬件清单中没有这两只电阻，必须新买并接线，否则电量显示将读到乱值。
>
> | 需要新购 | 规格 | 数量 | 参考价 |
> |---------|------|------|--------|
> | 电阻 R1 | **220kΩ**，1/4W，精度 1%（或 5% 也可）| 1 只 | ¥0.1 左右 |
> | 电阻 R2 | **47kΩ**，1/4W，精度 1%（或 5% 也可）| 1 只 | ¥0.1 左右 |
>
> 两只电阻淘宝/拼多多按"1% 金属膜电阻 220k 47k"搜索，通常 ¥2–5 可买到一大袋，不需要单独购买。
...
12V 电池 ──┬──→ ZK-502C VCC（直接 12V 供电）
           ...
           │
           └──→ 电池电量分压器（新增接线）：
                  12V+ ──[R1 220kΩ]──┬──[R2 47kΩ]── GND（与 ESP32 共地）
                                      │
                                      └──→ GPIO7（ESP32-S3 PIN_BATT_ADC）
```

- 分压后：12.6V 满电 → GPIO7 ≈ **2.22V**（在 ADC 3.3V 量程内 ✓）
- 分压后：9.0V 放空 → GPIO7 ≈ **1.59V**（仍在量程内 ✓）
- 三个焊点：电池正极一端、GPIO7 中间节点、GND 一端
- 无需精密电阻：5% 普通碳膜电阻即可，显示精度误差约 ±2%，不影响使用
